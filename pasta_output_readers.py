...
class pasta_job:
    ...
    def make_pbs_file(self, queue='secondary', append_to=None):
        ...
        if queue == 'secondary':
            ...
            for line in tmpf:
                ...
                a = a.replace('SFTP_COMMAND_FILE',
                              self.cfg.get('pasta_batch_config', 'cc_sftp_folder') + self.job_name + '_sftp.txt')
                a = a.replace('PASTA_TEMP_FOLDER',
                              self.cfg.get('pasta_batch_config', 'cc_pasta_temp_folder') + self.job_name)
        ...
        if queue == 'stat':
            if append_to == None:
                ...
                tmpf = open(self.prefix +'/code/phylogeny_utilities/pasta_pbs_main_commands.txt', 'r')
                for line in tmpf:
                    a = line.replace('PATH_TO_INPUT_FILE', self.path_to_input_file)
                    a = a.replace('JOB_NAME', self.job_name)
                    a = a.replace('PATH_TO_RESULTS_FOLDER', self.cc_results_folder)
    ...
    def make_sftp_file(self, garbage=False):
        if garbage == True:
            myfile = self.prefix + self.cfg.get('pasta_batch_config',
                                                'sftp_folder_garbage') + self.job_name + '_sftp.txt'
        else:
...
def main2(cfg):
    ...
    try:
        ...
        reg2 = None
    batchsubf = open(
        prefix + cfg.get('pasta_batch_config', 'results_folder') + 'batch_submit_' + cfg.get('pasta_batch_config',
                                                                                             'data_set_name') + '.sh',
        'w')
    ...
    for i in newpastas.aln_tree_files_true:
        ...
        for j in sizes:
            ...
            if k==1:
                myjob=job_name
                qsub = mypastajob.make_pbs_file('stat')
            else:
                qsub = mypastajob.make_pbs_file('stat',myjob)
    ...
    sftp.write('mput ' + prefix + cfg.get('pasta_batch_config', 'results_folder') + 'batch_submit_' + cfg.get(
        'pasta_batch_config', 'data_set_name') + '.sh\n')
...
class groupOfCompletedPastaRuns:
    ...
    def make_results_sftp_files(self):
        aln_batch_zip=self.pf + self.cfg.get('pasta_batch_config', 'results_folder') + 'zip_all_aln_trees.sh'
        aln_sftp = self.pf + self.cfg.get('pasta_batch_config', 'results_folder') + 'sftp_commands_alignment.txt'
        af = open(aln_sftp, 'w')
        az = open(aln_batch_zip,'w')
    ...
    def get_pasta_job_name(self, pasta_run=None, which_os=None):
        ...
        if len(jobs) > 1:
            print 'Job ' + pasta_run['name'] + ' in folder ' + self.prefixes['results_prefix'] + pasta_run[
                'results_folder'] + ' contains results from multiple pasta runs:'
            print jobs
            return None
        elif jobs == None or len(jobs) == 0:
            print 'Job ' + pasta_run['name'] + ' in folder ' + self.prefixes['results_prefix'] + pasta_run[
                'results_folder'] + ' does not contain any valid console output files.'
            return None
        else:
            return jobs[0]
    ...
    def make_tree_batch_file_row(self, pasta_run=None, pasta_output=None, which_os='Windows', iteration=None):
        ...
        myargs['outputloc'] = prefix + self.cfg.get('results', 'tree_error_loc') + 'tree_error_' + pasta_output[
            'job'] + '.txt'
    ...
    def make_alignment_batch_file_row(self, pasta_run=None, pasta_output=None):
        ...
        myargs['outputloc'] = self.pf + self.cfg.get('results', 'aln_error_loc') + 'aln_error_' + pasta_output[
            'job'] + '.txt'
    ...
    def assemble_all_results(self, file_name):
        ...
        rf.write(
            'job\tmodel\tset\tsize\twall_time\tsplitsfound\tsplits_total\tsplits_frac\tsplts_maxlndf\tsplits_ratio\tfile_name\tSP-Score\tModeler\tSPFN\tSPFP\tCompression Ratio\tTC\tMaxLenNoGap\tNumSeq\tLenRef\tLenEst\tCells\t\n')
    ...
    def assemble_times_results(self, file_name=None):
        ...
        outf.write(
            'file\tinitalign_mafft\tinitalign-hmmeralign\tinittree_fasttree\taln0\titer0-centroid\taln1\titer1-centroid\taln2\titer2-centroid\n')
        k = 1
        n = len(files)
        for l in files:
...
def read_pasta_times_log_file(filepath=None):
    ...
    for l in myf:
        ...
        if myreset <> None:
            cats = {'initalign_mafft': [], 'initalign-hmmeralign': [], 'inittree_fasttree': [], 'iter0-centroid': [],
                    'iter1-centroid': [], 'iter2-centroid': []}
...
def read_CompareTree_results_file(filepath=None):
    myf = open(filepath, 'r')
    dataregs = re.compile(
        'Splits	Found	(?P<splitsfound>\d+)	Total	(?P<total>\d+)	Frac	(?P<frac>\d+\.*\d*)	MaxLnDf	(?P<maxlndf>\d*\.*\d*)	Ratio	(?P<ratio>\d*\.*\d*)	MaxBtDf')
    text = myf.read()
    myf.close()
